    ...
    <div>
      ...
      <section className="relative pt-20 pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center px-3 py-1 rounded-full bg-indigo-50 text-accent text-xs font-bold uppercase tracking-wide mb-8">
            New: AI-Powered SEO Audits
          </div>
          <h1 className="text-5xl md:text-7xl font-extrabold text-slate-900 tracking-tight mb-8 leading-tight">
            Beautifully Engineered <br className="hidden md:block" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">Websites That Convert</span>
          </h1>
          ...
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16 opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
             {TECH_STACK.map((tech) => (
               <div key={tech.name} className="flex items-center gap-2 group">
                 <tech.icon size={24} className="text-slate-800 group-hover:text-accent" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {SERVICES.slice(0, 6).map((service) => {
              ...
              return (
                <Link key={service.id} to={`/services/${service.slug}`} className="group block p-8 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all">
