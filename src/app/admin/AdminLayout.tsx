...
const NAV_ITEMS = [
  ...
  { to: "/admin/leads", label: "Inquiries", icon: MessageSquare },
  { to: "/admin/projects", label: "Projects", icon: FolderKanban },
  { to: "/admin/blog", label: "Journal", icon: BookOpen },
  { to: "/admin/case-studies", label: "Case Studies", icon: Users },
];
...
export function AdminLayout() {
  ...
  if (!token) {
    return (
      <div style={{ minHeight: "100vh", backgroundColor: "#080808", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ position: "absolute", top: "20%", left: "50%", transform: "translateX(-50%)", width: "600px", height: "400px", background: "radial-gradient(ellipse, rgba(196,164,107,0.04) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: "420px", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: "48px" }}>
            <span style={{ fontFamily: "'Instrument Serif', serif", fontSize: "32px", color: "#F0EBE0", fontStyle: "italic", letterSpacing: "-0.02em", display: "block", marginBottom: "8px" }}>Æ</span>
            <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "9px", fontWeight: 500, color: "rgba(240,235,224,0.35)", letterSpacing: "0.18em", textTransform: "uppercase" }}>Studio Admin</span>
          </div>
          <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", padding: "40px", position: "relative" }}>
            <div style={{ position: "absolute", top: 0, left: "20%", right: "20%", height: "1px", background: "linear-gradient(90deg, transparent, rgba(196,164,107,0.5), transparent)" }} />
            <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: "28px", color: "#F0EBE0", fontStyle: "italic", letterSpacing: "-0.02em", marginBottom: "6px", lineHeight: 1.2 }}>Welcome back</h1>
            <p style={{ fontSize: "12px", color: "rgba(240,235,224,0.4)", letterSpacing: "0.02em", marginBottom: "36px" }}>Sign in to your studio dashboard</p>
            <form onSubmit={login} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                <label style={{ fontSize: "10px", fontWeight: 500, color: "rgba(240,235,224,0.45)", letterSpacing: "0.1em", textTransform: "uppercase" }}>Password</label>
                <div style={{ position: "relative", borderBottom: `1px solid ${focused === "password" ? "rgba(196,164,107,0.6)" : "rgba(255,255,255,0.1)"}`, transition: "border-color 0.3s ease", paddingBottom: "10px", display: "flex", alignItems: "center" }}>
                  <input type="password" value={input} onChange={(e) => setInput(e.target.value)} onFocus={() => setFocused("password")} onBlur={() => setFocused(null)} placeholder="••••••••••" autoFocus style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: "14px", color: "#F0EBE0", fontFamily: "'Inter', sans-serif", caretColor: "#C4A46B" }} />
                </div>
              </div>
              {error && <p style={{ color: "#f87171", fontSize: "12px", margin: 0, letterSpacing: "0.02em" }}>{error}</p>}
              <button type="submit" disabled={checking} style={{ width: "100%", padding: "14px 24px", background: "#F0EBE0", border: "none", cursor: checking ? "not-allowed" : "pointer", fontFamily: "'Inter', sans-serif", fontSize: "11px", fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", color: "#080808", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", marginTop: "8px", transition: "background 0.2s ease", opacity: checking ? 0.6 : 1 }}>
                {checking ? "Signing in..." : "Sign In"}
              </button>
            </form>
          </div>
          <p style={{ textAlign: "center", marginTop: "28px", fontSize: "10px", color: "rgba(240,235,224,0.2)", letterSpacing: "0.05em" }}>Aesthetix Studio · Internal Platform</p>
        </div>
      </div>
    );
  }
