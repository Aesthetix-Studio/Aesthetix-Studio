...
const EMPTY: Omit<Project, "id"> = {
  name: "", slug: "", category: "", summary: "", result: "",
...
export function ProjectsAdmin() {
      ...
      <div>
        ...
        <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", padding: "32px", maxWidth: 640, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {(["name", "slug", "category", "summary", "result", "image"] as const).map(field => (
  ...
  return (
    <div>
      <ConfirmDialog open={deleteTarget !== null} title="Delete project?" message={`This will permanently delete "${deleteTarget?.name}" and cannot be undone.`} onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} />
      ...
      <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        {projects.map((p, i) => (
          <div key={p.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 80px 120px", padding: "16px 24px", borderBottom: i < projects.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", alignItems: "center", gap: "16px", transition: "background 0.2s ease" }}
            ...
            <div><div style={{ fontSize: "13px", fontFamily: "'Instrument Serif', serif", fontStyle: "italic", color: "rgba(240,235,224,0.85)", marginBottom: "2px" }}>{p.name}{p.is_featured ? <span style={{ fontSize: "9px", color: "#C4A46B", marginLeft: "6px", fontFamily: "'Inter', sans-serif", fontStyle: "normal" }}>★</span> : null}</div></div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              ...
              <button onClick={() => setDeleteTarget({ slug: p.slug, name: p.name })} style={s.danger}>Delete</button>
            </div>
          </div>
        ))}
      </div>
